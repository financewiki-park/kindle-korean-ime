...
int main(int argc,char **argv) {
    void *lib;X11Api x;Display *d;Window w;int revert,min,max,preedit=0,korean=1;HangulState s;
    ...
    for(;;){XEvent e;KeySym sym;HangulResult r;int i;x.get_input_focus(d,&w,&revert);if(w==XNone)continue;x.select_input(d,w,XKeyReleaseMask);x.next_event(d,&e);if(e.type!=XKeyRelease)continue;if(e.xkey.keycode==(unsigned int)max||e.xkey.keycode==(unsigned int)(max-1))continue;sym=x.lookup_keysym(&e.xkey,0);if(sym==XK_space&&(e.xkey.state&ShiftMask)){korean=!korean;hangul_reset(&s);preedit=0;continue;}if(!korean)continue;if(sym==XK_BackSpace)r=hangul_backspace(&s);else if(sym>=0x3131&&sym<=0x3163)r=hangul_feed(&s,(uint32_t)sym);else{hangul_reset(&s);preedit=0;continue;}for(i=0;i<1+preedit;i++)send_key(&x,d,w,0x16,XKeyPressMask|XKeyReleaseMask);if(r.commit)inject(&x,d,w,(KeySym)r.commit,max-1);if(r.preedit)inject(&x,d,w,(KeySym)r.preedit,max);preedit=r.preedit!=0;x.flush(d);}
}
